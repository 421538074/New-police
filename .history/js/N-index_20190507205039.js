...
function getNotice() {
  $.ajax({
    ...
    dataType: 'json',
    success: function success(res) {
      vm.list = res.data.comment;
      vm.replaylist = res.data.replay;
    }
  });
}
/**
...
function getRepairList(strDate) {
  ...
  // Ajax.post("".concat(api, "/index/api/repairCentre"),{today: nowDate,begin: dateRange.start,end: dateRange.end},function(e) {
  //   console.log(JSON.stringify(e));
  // });
  var xhr = new XMLHttpRequest();
    xhr.open("POST", "".concat(api, "/index/api/repairCentre"), true);
    // 添加http头，发送信息至服务器时内容编码类型
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");  
    xhr.onreadystatechange = function() {
      if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 304)) {
  ...
  //     vm.repairInfo.today = data.data.today;
  //     vm.repairInfo.week = data.data.week;
